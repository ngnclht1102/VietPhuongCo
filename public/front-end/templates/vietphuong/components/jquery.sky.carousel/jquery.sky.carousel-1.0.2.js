...
SKY.CarouselItem.createReflection = function (a, b, c) {
    ...
    SKY.Utils.hasCanvasSupport() ? (e = $("<canvas>"), ctx = e.get(0).getContext("2d"), e.attr({
        width: d,
        height: b
    }), e.addClass("reflection"), ctx.save(), ctx.translate(0, g), ctx.scale(1, -1), ctx.drawImage(a.get(0), 0, 0, d, g), ctx.restore(), ctx.globalCompositeOperation = "destination-out", a = ctx.createLinearGradient(0, 0, 0, b), a.addColorStop(0, "rgba(0, 0, 0, " + (1 - c) + ")"), a.addColorStop(1, "rgba(0, 0, 0, 1.0)"), ctx.fillStyle = a, ctx.fillRect(0, 0, d, b)) : (e = $("<img>"), e.attr({
        src: a.get(0).src
...
SKY.Carousel.prototype = {
    ...
    update: function () {
        for (var a = this, b = this.settings, c = this.container, d = c.getLeft(), g = [], e = 0; e < this.carouselItems.length; e++) {
